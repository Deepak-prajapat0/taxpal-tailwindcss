...
function Header() {
  ...
  return (
    <header className="p-5 border  text-gray-700">
      <nav className="flex flex-row justify-between items-center ">
        <div className="flex gap-10">
          ...
          <ul className=" md:flex lg:flex flex-row gap-4 items-center text-md hidden ">
            {links.map((link, index) => {
              return (
                <li key={index}>
                  <Link className="hover:text-black" to={link.path}>{link.name}</Link>
                </li>
              );
            })}
          </ul>
        ...
        <div className="flex gap-10 ">
          <button className="max-sm:hidden hover:text-black " onClick={() => navigate("/login")}>
            Sign in
          </button>
          <button className=" p-2 px-3 rounded-3xl text-white bg-[#2563EB] hover:bg-[#053cb3]"  onClick={() => navigate("/register")}>
            Get started today
          </button>
        </div>
      </nav>
